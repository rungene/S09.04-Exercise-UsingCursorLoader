...
//      TODO (7) Generate a weather summary with the date, description, high and low
        ...
        //read high temperatures from the cursor (in degress celsius)
        double highInCelsius = mCursor.getDouble(MainActivity.INDEX_WEATHER_MAX_TEMP);
        //Read low temperature from the cursor (in degree celsius)
        double lowInCelsius = mCursor.getDouble(MainActivity.INDEX_WEATHER_MIN_TEMP);

        String highAndLowTemperatures =
                SunshineWeatherUtils.formatHighLows(mContext,highInCelsius,lowInCelsius);

        String weatherSummary = dateString + "-"+description+" -"+ highAndLowTemperatures;
